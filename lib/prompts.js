...
module.exports = {
  ...
  finalConfirmation(targetPath) {
    return new Promise((resolve) => {
      console.log('Final confirmation. If you wish to proceed, please type in the name of your target directory.');
      prompt.start();
      prompt.get(['name'], (err, result) => {
        const targetedName = path.basename(targetPath);
        if (targetedName !== result.name) {
          console.log(`The name you entered was ${result.name}, but your targeted directory was ${targetedName}. Terminating.`);
          process.exit();
        } else {
          resolve(true);
        }
